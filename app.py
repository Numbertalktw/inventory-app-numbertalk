...
if page == "📦 商品管理":
    ...
    with t2:
        ...
        if not df_p_formatted.empty:
            ...
            sel_label = st.selectbox("🔍 選擇商品 (可搜尋品名)", options=all_labels)
...
# --- 🔨 製造作業 ---
    ...
    with t1:
        ...
        wh = c2.selectbox("發料倉庫", WAREHOUSES, key="mwh"); qty = c3.number_input("數量", min_value=1.0, value=1.0, key="mqty")
    ...
    with t2:
        with st.form("m2_form"):
            sel_out = st.selectbox("成品", prods['label']); wh_out = st.selectbox("入庫倉庫", WAREHOUSES); qty_out = st.number_input("數量", 1.0)
