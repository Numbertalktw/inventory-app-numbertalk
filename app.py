            ...
            with st.form("edit_prod"):
                ...
                if st.form_submit_button("💾 儲存修改"):
                    if update_product(sel_sku, {'name': n_name, 'spec': n_spec, 'color': n_color, 'note': n_note}):
                        st.success("✅ 修改成功"); clear_cache(); time.sleep(1); st.rerun()
    ...
    if not prods.empty:
        with st.form("tr_form"):
            ...
            if st.form_submit_button("🚀 執行移庫"):
                if f_wh == t_wh: st.error("❌ 倉庫不能相同")
                else:
                    sku = sel_p.split(" | ")[0]
                    add_transaction("移庫(撥出)", str(date.today()), sku, f_wh, qty, user, f"移至 {t_wh}")
                    add_transaction("移庫(撥入)", str(date.today()), sku, t_wh, qty, user, f"來自 {f_wh}")
    ...
    if not prods.empty:
        with st.form("in_form"):
            ...
            if st.form_submit_button("執行進貨"):
                if add_transaction("進貨", str(date.today()), sel_p.split(" | ")[0], wh, qty, user, ""):
...
# --- 🚚 出貨作業 ---
    ...
    c1, c2 = st.columns(2)
    order_id = c1.text_input("客戶備註 / 訂單號碼", placeholder="例如: James #3840")
    user = c2.selectbox("經手人", KEYERS, index=3)
    detail_note = st.text_area("📋 詳細備註", height=100)
    prods = get_formatted_product_df()
    if not prods.empty:
    ...
    if st.session_state['out_list']:
        ...
        for i, item in enumerate(st.session_state['out_list']):
            ...
            ct.write(f"🔸 **{item['name']}** - {item['wh']} x{item['qty']}")
            if cd.button("❌", key=f"rm_o_{i}"): st.session_state['out_list'].pop(i); st.rerun()
        combined = f"{order_id} | {detail_note}".strip(" | ")
        if st.button("✅ 批次確認出貨", type="primary", use_container_width=True):
            for x in st.session_state['out_list']:
                add_transaction("銷售出貨", str(date.today()), x['sku'], x['wh'], x['qty'], user, combined)
...
# --- 🔨 製造作業 ---
    ...
    with t1:
        m_note = st.text_input("領料總備註", placeholder="製作項目說明", key="m_note_batch")
        c1, c2, c3 = st.columns([3, 1, 1])
        sel = c1.selectbox("原料", prods['label'], key="msel")
        wh = c2.selectbox("發料倉庫", WAREHOUSES, key="mwh")
        qty = c3.number_input("數量", min_value=1.0, value=1.0, key="mqty")
        ...
        if st.session_state['m_in_list']:
            for i, item in enumerate(st.session_state['m_in_list']):
                ct, cd = st.columns([5, 1])
                ct.write(f"🔸 **{item['name']}** - {item['wh']} x{item['qty']}")
                if cd.button("❌", key=f"rm_m_{i}"): st.session_state['m_in_list'].pop(i); st.rerun()
    ...
    with t2:
        with st.form("m2_form"):
            ...
            wh_out = st.selectbox("入庫倉庫", WAREHOUSES)
            qty_out = st.number_input("數量", 1.0)
            if st.form_submit_button("完工確認"):
                add_transaction("製造入庫", str(date.today()), sel_out.split(" | ")[0], wh_out, qty_out, "工廠", "")
    ...
    with t3:
        ...
        with c1:
            with st.form("d1_form"):
                p = st.selectbox("成品", prods['label'], key="dp")
        ...
        with c2:
            with st.form("d2_form"):
                m = st.selectbox("原料", prods['label'], key="dm")
