...
def update_product(sku, new_data):
    ...
    try:
        ...
        st.error(f"更新失敗: {e}"); return False
...
if page == "📦 商品管理":
    ...
    with t2:
        ...
        if not df_p_formatted.empty:
            ...
            sel_label = st.selectbox("🔍 選擇商品 (可輸入品名或貨號搜尋)", options=all_labels)
            ...
            with st.form("edit_prod_form"):
                ...
                if st.form_submit_button("💾 儲存修改"):
                    if update_product(sel_sku, {'name': n_name, 'spec': n_spec, 'color': n_color, 'note': n_note}):
                        st.success("✅ 更新成功"); clear_cache(); time.sleep(1); st.rerun()
        else: st.info("尚無商品資料。")
...
# --- 🚚 出貨作業 ---
    ...
    if st.session_state['out_list']:
        ...
        for i, item in enumerate(st.session_state['out_list']):
            ...
            ct.write(f"🔸 **{item['name']}** ({item['sku']}) - {item['wh']} x{item['qty']}")
            if cd.button("❌", key=f"rm_o_{i}"):
...
# --- 🔨 製造作業 ---
    ...
    with t1:
        ...
        wh = c2.selectbox("倉庫", WAREHOUSES, key="mwh")
