...
PREFIX_MAP = {
    ...
    "天然石": "NS", "金屬配���": "MT", "線材": "WR", "包裝材料": "PK", "完成品": "PD"
}
...
def update_order_status(order_no, new_status):
    ...
    if not ws:
        st.error("無法連線到 Orders 工作表，請稍後重試")
        return False
    try:
        all_vals = ws.get_all_values()
        header = all_vals[0]
...
def ship_order(order_no, keyer, ship_method="", ship_no="", target_status="未付款/已出貨"):
    ...
    for _, item in items.iterrows():
        ...
        if not ok:
            return False, f"品項 {item['sku']} 出貨失���"
...
with st.sidebar:
    st.header("功能選單")
    page = st.radio("前往", ["🛒 訂單管理", "👥 會員管��", "🔨 製造作業", "🚚 出貨作業", "📦 商品管理", "📥 進貨作業", "📦 移庫作業", "📊 報表查詢"])
    if st.button("刷新資料"):
        clear_cache()
        st.rerun()
...
if page == "📦 商品管理":
    ...
    with t1:
        ...
        final_cat = c_cat.text_input("新分類名稱") if cat_opt == "手��輸入新分類..." else cat_opt
        ...
        with st.form("in_form"):
            ...
            if st.form_submit_button("執行進貨"):
                ...
                if add_transaction("進貨", date.today(), sku_only, wh, qty, user, in_note, cost=in_cost):
                    st.success("進貨��功"); time.sleep(1); st.rerun()
...
# --- 🛒 訂單管理 ---
    ...
    with tab_new:
        ...
        prev_sel = st.session_state.get('_prev_member_sel', "-- 手動輸��� --")
        ...
        cust_name = cc1.text_input("客戶���稱 *必填", key="o_cname")
        ...
        if st.session_state['order_items']:
            ...
            if o_discount > 0 or o_ship_fee > 0:
                st.caption(f"商品 ${items_total:,.0f} - 折扣 ${o_discount:,.0f} + ��費 ${o_ship_fee:,.0f}")
    ...
    with tab_list:
        ...
        if df_orders.empty:
            ...
            sub_pending, sub_done = st.tabs(["📋 未完成", "✅ 已��成"])
            ...
            def render_order_list(df_filtered, kp):
                ...
                for _, row in df_filtered.iterrows():
                    ...
                    with st.expander(f"{icon} {ono} | {row.get('customer_name', '')} | ${row.get('total_amount', 0):,.0f} | {status}"):
                        ...
                        dc1.write(f"日���: {row.get('order_date', '')}")
                        ...
                        if options:
                            ...
                            if need_ship:
                                ...
                                s_method = sc2.selectbox("寄送��式", SHIPPING_METHODS, key=f"{kp}_sm_{ono}")
                            ...
                            if ac1.button(btn_label, key=f"{kp}_apply_{ono}", type="primary"):
                                if need_ship:
                                    ...
                                    if update_order_status(ono, new_st):
                                        ...
                                        time.sleep(1)
                                        st.rerun()
                                    # 失敗時 update_order_status 內部已顯示 st.error

                            # 刪除按鈕(僅限已成立/待處理/已取消)
                            if status in ["已成���", "待處理", "已取消"]:
            ...
            with sub_pending:
                pending = df_orders[~df_orders['status'].isin(["已完成", "���取消"])]
    ...
    with tab_detail:
        ...
        if not df_orders.empty:
            ...
            sel_order = st.selectbox("選��訂單", order_labels, key="o_detail_sel")
            ...
            if not row.empty:
                ...
                if not items.empty:
                    ...
                    st.dataframe(
                        items[['sku', 'product_name', 'qty', 'unit_price', 'subtotal', 'warehouse']].rename(
                            columns={'sku': '貨號', 'product_name': '品名', 'qty': '數量',
                                     'unit_price': '單價', 'subtotal': '��計', 'warehouse': '倉庫'}
                        ),
                        use_container_width=True, hide_index=True
                    )
        else:
...
# --- 👥 會��管理 ---
    ...
    with tab_m_list:
        ...
        if df_members.empty:
            ...
            st.dataframe(
                filtered_m[['name', 'phone', 'email', 'address', 'last_order_date']].rename(
                    columns={'name': '姓名', 'phone': '電話', 'email': 'Email',
                             'address': '地址', 'last_order_date': '最後訂單��期'}
                ),
                use_container_width=True, hide_index=True
            )
            ...
            if m_names:
                sel_m = st.selectbox("選擇會��", m_names, key="m_edit_sel")
                m_data = find_member_by_name(sel_m)
                if m_data is not None:
                    with st.form("edit_member"):
                        em_phone = st.text_input("��話", value=str(m_data.get('phone', '')))
    ...
    with t3:
        ...
        with c1:
            with st.form("d1_form"):
                ...
                if st.form_submit_button("1. 扣除���品"):
                    add_transaction("製造入��", date.today(), p.split(" | ")[0], "Wen", -q, "管理員", "拆解扣除")
                    st.success("OK"); time.sleep(1); st.rerun()
        with c2:
            with st.form("d2_form"):
                m = st.selectbox("原料", prods['label'], key="dm"); q = st.number_input("回庫量", 1.0)
                if st.form_submit_button("2. 回庫原料"):
                    add_transaction("製造領料", date.today(), m.split(" | ")[0], "Wen", -q, "管理���", "拆解回庫")
                    st.success("OK"); time.sleep(1); st.rerun()
